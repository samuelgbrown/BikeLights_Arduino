...
class Pattern
{
  public:
    ...
    // Functions to manage the "colors" array of Color_**'s, or the Color_'s that this pattern may show
    void setupColors(unsigned char numColors);
    void setupColors(Color_** colorsIn, unsigned char numColorsIn);
    void setColor(Color_* newColor, unsigned char colorNum); // Set a color in the specified location

    unsigned char getNumColors(); // Get the number of colors currently assigned to this pattern
  ...
  protected:
    ...
    unsigned char numColors; // The number of colors is defaulted to 0 (Max of 255 Color_'s)

    void preCalculateAllColor_(); // Pre-calculate all colorObj's from each Color_ being used this loop
    colorObj getPreCalculatedColorInPos(unsigned char colorNum); // Get a specific colorObj according to its position in colors (If using this function, MUST have run preCalculateAllColor_() earlier in the loop)

    //    void serialWriteAllColors(); // Write all colors to the output

  private:
    void deleteColorArray(); // Delete the array colors and its contents

    Color_** colors = NULL; // Array of Color_ objects (0 will always be a black Static_Color object, which is treated as "empty" where applicable)
    unsigned char image[NUMLEDS]; // An array of integers, each of which represents a "color index", or the index in the colors array that represents the color desired
    colorObj * preCalculatedColors = NULL; // Array (of size numColors) of colorObj's that represents the colorObj for each Color_ represented in image that is being used this loop (each Color_ is calculated only once, so computation time will be saved on dynamic colors).
};
...
//    void sendColors(); // Send the colors in colorMemory to the LEDs
};
